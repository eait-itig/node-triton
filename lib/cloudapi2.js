...
CloudApi.prototype._request = function _request(opts, cb) {
    ...
    if (opts.headers && opts.headers.upgrade)
        clientFnName = 'request';
    ...
    self._getAuthHeaders(method, opts.path, function (err, headers) {
        ...
        if (clientFnName === 'request') {
            reqOpts = self.client._options(method.toUpperCase(), reqOpts);
            self.client.request(reqOpts, cb);
        } else if (opts.data) {
 ...
 * @param {Function} callback of the form `function (err, res)`
 */
CloudApi.prototype.machineExecInteractive =
    function machineExecInt(uuid, argv, callback) {
    ...
    this._request(opts, function (err, req, socket, head) {
        if (err) {
            callback(err);
            return;
        }
        req.end();
        req.once('upgradeResult', function (err2, res2, socket2, head2) {
            if (err2) {
                callback(err2);
                return;
            }
            var ws = shed.connect(res2, socket2, head2, wskey);
            var line = JSON.stringify({argv: argv}) + '\n';
            ws.send(new Buffer(line, 'utf-8'));
            callback(null, ws);
        });
    });
};
